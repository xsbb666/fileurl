...
$file_path = $http_file.'?fid='.$_GET["fid"].'&fkey='.$_GET["fkey"].'&uid='.$uid."&from=".$_GET["from"];
$file_path2 = $http_file.'?fid='.$_GET["fid"].'&fkey='.$_GET["fkey"].'&uid='.$_GET["uid"]."&from=".$_GET["from"];
...
$file = $redis->get($file_path);
if(!$file)$file = $redis->get($file_path2);

if($file){$filepath='../upload/'.$file;}

if($file&&!is_dir($filepath)&&file_exists($filepath)&&$http_file==$file) 
{
